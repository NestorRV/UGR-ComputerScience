...
; Listado de personas de mi familia introduccidas con la propiedad unaria de hombre o mujer

(deffacts personas
   (hombre JuanLuis) ; "JuanLuis es un hombre"
   (hombre Luis)
   (hombre Juan)
   (hombre Pepito)
   (hombre JoseEmilio)
...
(deffacts relaciones
   (Relacion (tipo HIJO) (sujeto Luis) (objeto JuanLuis)) ; "Luis es HIJO de JuanLuis
   (Relacion (tipo HIJO) (sujeto Julia) (objeto JuanLuis))
   (Relacion (tipo HIJO) (sujeto JuanLuis) (objeto Jose))
   ...
   (Relacion (tipo ESPOSO) (sujeto JuanLuis) (objeto Laura)) ; "JuanLuis es ESPOSO de Laura"
...
(defrule dualidad
   (buscar)
   (Relacion (tipo ?r) (sujeto ?x) (objeto ?y)(explicacion ?e))
   (dual ?r ?t)
=> 
   (assert (Relacion (tipo ?t) (sujeto ?y) (objeto ?x) (explicacion ?e))))
...
(defrule composicion
   ...
   (test (neq ?y ?z))
=> 
   (assert (Relacion (tipo ?u) (sujeto ?y) (objeto ?z) (explicacion (str-cat ?y " es " ?r "/A de " ?x ", y " ?x " es " ?t "/A de " ?z ".")))))
...
=>
   (printout t "Dime el nombre de la primera persona de la Familia Castro sobre la que quieres información (escribe solo el nombre): " crlf)
...
(defrule relacionmasculino
   ...
   (primerapersona ?x)		
   (segundapersona ?y)
   (Relacion (tipo ?r) (sujeto ?y) (objeto ?x)(explicacion ?e))
   (hombre ?y)
 =>
   (printout t ?y " es " ?r " de " ?x ". Porque " ?e crlf)
   (retract ?f)     )
...
(defrule relacionfemenino
   ...
   (femenino ?r ?t)
 =>
   (printout t ?y " es " ?t " de " ?x ". Porque " ?e crlf)
   (retract ?f)   )
