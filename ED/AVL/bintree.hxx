 ...
 * Author: nestor
 *
 * Created on 8 de enero de 2016, 11:48
 */
...
template <typename T>
typename bintree<T>::inorder_iterator
bintree<T>::inorder_iterator::operator++(int) {
    bintree<T>::inorder_iterator tmp;
    tmp = *this;
    if (elnodo.null())
        return tmp;
...
template <typename T>
typename bintree<T>::const_inorder_iterator
bintree<T>::const_inorder_iterator::operator++(int) {
    bintree<T>::const_inorder_iterator tmp;
    tmp = *this;
    if (elnodo.null())
        return tmp;
